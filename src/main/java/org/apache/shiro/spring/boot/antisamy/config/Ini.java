...
public class Ini implements Map<String, Ini.Section> {
    ...
    private static String cleanName(String sectionName) {
        String name = StringUtils.trimWhitespace(sectionName);
    ...
    private void addSection(String name, StringBuilder content) {
        if (content.length() > 0) {
            ...
            String cleaned = StringUtils.trimWhitespace(contentString);
    ...
    public void load(Scanner scanner) {
        ...
        while (scanner.hasNextLine()) {
            ...
            String line = StringUtils.trimWhitespace(rawLine);

            if (!StringUtils.hasText(line) || line.startsWith(COMMENT_POUND) || line.startsWith(COMMENT_SEMICOLON)) {
    ...
    public static class Section implements Map<String, String> {
        ...
        protected static String[] splitKeyValue(String keyValueLine) {
            ...
            String key = StringUtils.trimWhitespace(keyBuffer.toString());
            String value = StringUtils.trimWhitespace(valueBuffer.toString());
		...
		private static Map<String, String> toMapProps(String content) {
            ...
            while (scanner.hasNextLine()) {
                line = StringUtils.trimWhitespace(scanner.nextLine());
                if (isContinued(line)) {
